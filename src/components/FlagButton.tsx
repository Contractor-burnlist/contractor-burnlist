    ...
    <>
      ...
      {open && (
        <div className="absolute right-0 top-full z-10 mt-1 w-64 rounded-lg border border-[#e5e7eb] bg-white p-3 shadow-lg">
          ...
          <select value={reason} onChange={(e) => setReason(e.target.value)} className="mb-2 w-full rounded border border-[#e5e7eb] px-2 py-1.5 text-xs outline-none">
            <option value="">Select reason</option>
            {REASONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
          </select>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Details (optional)" rows={2} className="mb-2 w-full rounded border border-[#e5e7eb] px-2 py-1.5 text-xs outline-none" />
